...
addLocalEngine(result, ...)   (lib/multimodal.ts — every AnalysisResult needs an `engine`)
        │
        ▼
useCaseStore().addCase(newCase)  →  POST /api/cases  →  POST /api/v1/complaints/intake
...
2. **At final submit (the one new piece of work this port adds):** `base64ToFile()`
   ...
   `TriageCase.evidence` *before* `addCase` is called.

## The fake-tracking-number guard

`useCaseStore().addCase()` silently returns the **unmodified local `TriageCase`** — carrying a
client-invented placeholder reference — when the backend POST fails or is unreachable, rather
than throwing. The only signal that a case was genuinely persisted is the presence of
`_uploadToken` on the response. `submitCase()` in `WhatsAppDemo.tsx` checks for it explicitly
and, when it's absent, tells the citizen plainly that nothing has been filed yet rather than
showing the placeholder reference as if it were real — showing a fake "filed" confirmation is
the one outcome in this whole flow that can't be recovered from after the fact.
...
## Recommended follow-up: make `/track` a real lookup (not built here)

The chat's final message links to `/track?ref=<reference>`, matching the reference-in-URL
pattern the Bhumika original already used. Today, `/track` (`components/CitizenDashboard.tsx`)
renders only from the *browser's own* local case list — it does not read `?ref=` at all, and
every complaint-read backend endpoint is officer-gated (no citizen auth exists yet — see
`docs/technical.md`'s "Honest current limitations"). So a case filed through this chat is
visible on `/track` in that same browser, but the link is not yet a portal-style lookup that
works from another device. The on-page notice on `/whatsapp` says this plainly, and this doc
should stay in sync with it if either changes.

This was **deliberately left as a citizen-auth/roadmap decision, not built as part of this
port** — a fresh agent's first instinct here is often to add a public
`GET /complaints/{reference}`, which would be wrong: references are only a ~900k-value space
...
Related: niriksh's intake response returns only `tracking_number`, not a URL
(`backend/app/modules/bhumika/service.py`), so every consumer (Bhumika's relay, this chat)
currently constructs the `/track` link itself from its own guess at the site's base URL. A
`tracking_url` field returned alongside `tracking_number`, built server-side, would make the
link identical everywhere by construction rather than convention — small and backward-compatible,
but out of scope here too.
