...
function LoginForm() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      window.location.assign(destination?.startsWith("/") ? destination : "/dashboard");
    } catch (reason) {
  ...
  return <main className="officer-login-page">
    <section className="officer-login-card">
      ...
      <form onSubmit={submit}>
        ...
        <label>Password<input type="password" value={password} onChange={event => setPassword(event.target.value)} autoComplete="current-password" minLength={8} required autoFocus/></label>
