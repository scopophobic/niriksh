...
import { AlertTriangle, ArrowRight, CheckCircle2, CircleDot, Fingerprint, LoaderCircle, LockKeyhole, ShieldCheck, Video } from "lucide-react";
...
export function PreventionIntelligence() {
  ...
  return <div className="page prevention-page">
    <header className="prevention-hero"><div><p>Intelligence overview</p><h1>Turn case memory into safer next steps.</h1><span>Find repeated signals, verify the pattern with a person, then use that reviewed intelligence to guide future cases and awareness.</span><div className="prevention-hero-actions"><Link href="/awareness" className="prevention-hero-action"><Video/>Open awareness studio <ArrowRight/></Link><span><LockKeyhole/>Nothing publishes automatically.</span></div></div><div className="prevention-boundary"><ShieldCheck/><strong>Human verification is required</strong><span>Patterns are advisory. They never establish an offender, guilt, or legal responsibility.</span></div></header>
    ...
    {patterns && <>
      ...
      <section className="impact-loop"><div className="impact-loop-heading"><div><p>THE PREVENTION LOOP</p><h2>Make impact visible without making it up.</h2><span>These are evidence checkpoints. They show what the system did and what still needs an outcome signal.</span></div><Link href="/awareness">Create a reviewed draft <ArrowRight/></Link></div><div className="impact-steps"><div className="impact-step complete"><CircleDot/><small>Observed</small><strong>{recurringIndicators} recurring signals</strong><span>Exact indicators found across cases.</span></div><div className="impact-connector"/><div className={`impact-step ${verified.length ? "complete" : "current"}`}><ShieldCheck/><small>Decided</small><strong>{verified.length} verified patterns</strong><span>Human review controls promotion.</span></div><div className="impact-connector"/><div className="impact-step"><LockKeyhole/><small>Acted</small><strong>Warnings and drafts</strong><span>Measure matches reviewed and guidance approved.</span></div><div className="impact-connector"/><div className="impact-step"><CircleDot/><small>Learned</small><strong>Outcome evidence</strong><span>Collect acknowledgement, intervention, or reduced repeat harm.</span></div></div><div className="impact-proof"><strong>What you can prove today</strong><span>Linked cases, shared indicators, review decisions, warning matches, and approved draft history.</span><strong>What you should measure next</strong><span>Was the warning reviewed? Was action taken? Did the same signal recur after intervention?</span></div></section>
