...
export function Dashboard() {
  ...
  const visible = useMemo(() => cases.filter(item => {
    ...
    const matchesFilter = filter === "All" || item.reviewCategory === filter || (filter === "needs-information" && item.status === "Needs information");
    return matchesSearch && matchesFilter;
  }), [cases, filter, search]);
  const folderCount = new Set(cases.map(item => item.reviewCategory)).size;
  ...
  return (
    <div className="page dashboard-page product-dashboard">
      ...
      <section className="metric-grid product-metrics">
        ...
        <div className="metric-card"><span className="metric-icon red"><FolderOpen size={20}/></span><div><span>Subject folders</span><strong>{folderCount}</strong><small>Human-selected complaint categories</small></div></div>
      ...
      <section className="queue-panel overview-cases">
        ...
        <div className="simple-queue-head">
          ...
          <div className="simple-filter-tabs"><button className={filter === "All" ? "active" : ""} onClick={() => setFilter("All")}>All</button>{REVIEW_CATEGORIES.map(item => <button key={item.id} className={filter === item.id ? "active" : ""} onClick={() => setFilter(item.id)}>{item.label}</button>)}<button className={filter === "needs-information" ? "active" : ""} onClick={() => setFilter("needs-information")}>Needs information</button></div>
        </div>
        <div className="case-table-wrap">
          <table className="case-table simple-case-table">
            <thead><tr><th>CASE</th><th>WHAT WAS REPORTED</th><th>SUBJECT FOLDER</th><th>STATUS</th><th>FILES</th><th></th></tr></thead>
        ...
        <div className="table-footer"><span>Showing {visible.length} of {cases.length} cases in received order</span><span>No automated priority ranking</span></div>
      </section>
      ...
      <aside className="analysis-engine-card">
        ...
        <p>The current engine reads complaint text, readable evidence text and user notes as separate sources. It does not score urgency, assign guilt, or decide routing.</p>
